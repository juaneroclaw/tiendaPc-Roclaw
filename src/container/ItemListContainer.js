...
export const ItemListContainer = () => {
    const {id} = useParams();
    const {items,itemsBusqID,tituloCategory,ifCat} = useContext(CartFirabase)
    ...
    return(
        <>
            {
            ifCat ? <ItemList item={prod} id={id} tituloCategory={tituloCategory}/> : <Row><Col className="text-center"><Image src={loading} className="loading"/></Col></Row>
            }
